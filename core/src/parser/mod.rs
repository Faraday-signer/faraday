...
pub fn build_review_lines(
    ...
    if parsed.has_unresolved_accounts {
        lines.push("! Some accounts resolve on-chain — unverifiable offline".to_string());
    }
...
mod tests {
    ...
    // SystemProgram::AdvanceNonceAccount, followed by a SOL transfer — the
    // shape Faraday builds so a slow QR relay can't expire the signature.
    // Generated by `just nonce-fixtures`. These pin, at the byte level, that
    ...
    fn has_header(ix: &ParsedInstruction, expected: &str) -> bool {
        ix.items
            .iter()
            .any(|item| matches!(item, ReviewItem::Header(h) if h == expected))
    }

    #[test]
    fn nonce_legacy_advance_is_first_labeled_instruction() {
        let parsed = parse(TX_NONCE_LEGACY);
        assert!(matches!(parsed.version, TransactionVersion::Legacy));
        assert_eq!(parsed.instructions.len(), 2);
        // First instruction is the labeled advance-nonce, not an unknown warning.
        assert_eq!(parsed.instructions[0].program, "System");
        assert!(
            has_header(&parsed.instructions[0], "Advance Nonce"),
            "first instruction must be labeled 'Advance Nonce'"
        );
        let first_warns = parsed.instructions[0]
            .items
            .iter()
            .any(|i| matches!(i, ReviewItem::Warning(_)));
        assert!(!first_warns, "advance-nonce must not warn");
        // The trailing transfer still parses as a SOL transfer.
        assert!(has_header(&parsed.instructions[1], "SOL Transfer"));
    }

    #[test]
    fn nonce_v0_advance_is_first_labeled_instruction() {
        let parsed = parse(TX_NONCE_V0);
        assert!(matches!(
            parsed.version,
            TransactionVersion::V0 {
                address_table_lookups: 0
    ...
    #[test]
    fn nonce_advance_shows_account_and_authority_rows() {
        let parsed = parse(TX_NONCE_LEGACY);
        let labels: Vec<&str> = parsed.instructions[0]
            .items
            .iter()
