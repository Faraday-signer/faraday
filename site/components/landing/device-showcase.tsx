...
function ScreenCycle({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div className="screen-cycle relative h-full w-full overflow-hidden bg-[oklch(0.19_0.02_240)]">
      {/* 1 — menu (mirrors the real UI) */}
      <div className="screen-frame absolute inset-0 flex flex-col p-2" style={{ animationDelay: "0s" }}>
        <p className={`font-mono ${text} uppercase tracking-widest text-brand`}>▚ Faraday</p>
        <div className="mt-1.5 border border-brand/60 bg-brand/15 px-1.5 py-1">
          <p className={`font-mono ${text} uppercase text-brand`}>Create · new wallet</p>
        </div>
      ...
      <div className="screen-frame absolute inset-0 flex flex-col p-2" style={{ animationDelay: "3s" }}>
        <p className={`font-mono ${text} uppercase tracking-widest text-brand`}>Review</p>
        ...
        <p className={`mt-auto font-mono ${text} uppercase text-brand`}>✓ Sign</p>
      </div>
      {/* 3 — signed QR */}
      <div className="screen-frame absolute inset-0 flex items-center justify-center p-2" style={{ animationDelay: "6s" }}>
        <QrGlyph className="h-4/5 w-auto text-brand" title="Signed transaction QR" />
      </div>
    </div>
  );
}
        ...
        {(
          ...
          <button
            ...
            className={`rounded-[1px] px-4 py-1.5 font-mono text-[11px] uppercase tracking-[0.16em] transition-colors ${
              device === value ? "bg-brand/10 text-brand" : "text-muted-foreground hover:text-foreground"
            }`}
          >
            {label}
          </button>
