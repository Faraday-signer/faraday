...
6./ Team board sync
The pinned message in the private "Faraday Signal" Telegram channel is the live board and the **authority for status** (who is working on what, right now); `docs/backlog.md` is the authority for card content (plans, acceptance criteria). Check the pin (`scripts/tg-board.sh read-pin`) alongside `gh pr list` before starting work on a card; post when you claim or finish one and refresh the pin (full board, one line per card — see docs/telegram-board.md). Requires `.env` (copy `.env.example`); if it's missing, say so and continue — a missing mirror never blocks work.
