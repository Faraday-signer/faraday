...
Faraday connects to dapps through the Solana Wallet Standard, so sites that already support Phantom, Backpack, or Solflare work without any changes. There are no Faraday servers: the extension talks directly to a public Solana RPC endpoint and to Jupiter's public token APIs to read balances and resolve token symbols and prices, and to nothing else. No account, no email address, no analytics, no telemetry, no tracking. Every transaction is decoded and shown to you twice — once in the extension, once on the device — before anything is signed.
